...
def attention_decoder_fn_train(encoder_state,
  ...
  Args:
    ...
    extra_information: other embeddings like latent samples.
    ...
    with ops.name_scope(
      ...
      if cell_state is None:  # first call, return encoder_state
        ...
        attention = attention_construct_fn(cell_output, attention_keys,
                                           attention_values) #batch*2units
        cell_output = attention

      # combine cell_input and attention
      next_input = array_ops.concat([cell_input, attention], 1) #batch*(embed_units + 2num_units + pattern_embed)
  ...
  def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
    ...
    with ops.name_scope(
      ...
      if cell_output is None:
        ...
        cell_output= output_fn(cell_output)  # probability on vocabulary list
